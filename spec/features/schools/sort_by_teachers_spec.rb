...
RSpec.describe 'As a visitor' do
  describe 'When I see the Schools Index Page' do
    it 'Has a link to sort schools by number of teachers' do
      ...
      visit "/schools"
      expect(page).to have_link("See Schools by Number of Employees")
    end

    it 'Takes me back to the School Index' do
      visit "/schools"
      click_link("See Schools by Number of Employees")
      expect(current_path).to eq("/schools")
    end

    it 'Shows School from most employees to least with number next to each school' do
